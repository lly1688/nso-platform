...
public class DocumentServiceImpl implements IDocumentService {
    ...
    public DocumentServiceImpl(DocumentVersionMapper documentVersionMapper, BomMapper bomMapper, BomItemMapper bomItemMapper,
                               ProcessRouteMapper routeMapper, ProcessStepMapper stepMapper, InspectionSpecMapper inspectionSpecMapper,
                               InspectionItemMapper inspectionItemMapper, FileObjectMapper fileObjectMapper, IFileService fileService, ProjectMapper projectMapper, BusinessLockPort businessLock, ProjectDataScope dataScope, BusinessEventService events, DocumentWorkflowProperties workflowProperties) {
        this.documentVersionMapper = documentVersionMapper; this.bomMapper = bomMapper; this.bomItemMapper = bomItemMapper;
        this.routeMapper = routeMapper; this.stepMapper = stepMapper; this.inspectionSpecMapper = inspectionSpecMapper;
        this.inspectionItemMapper = inspectionItemMapper; this.fileObjectMapper = fileObjectMapper; this.fileService = fileService; this.projectMapper = projectMapper;
        this.businessLock = businessLock;
        this.dataScope = dataScope; this.events = events; this.workflowProperties = workflowProperties;
    }
    ...
    public DocumentVersionDto createVersion(Long projectId, DocumentVersionRequest request) {
        ...
        version.setTenantId(TenantContext.tenantId()); version.setProjectId(projectId); version.setFileObjectId(file.getId()); version.setFileName(request.fileName().trim());
        version.setFileType(request.fileType().trim()); version.setVersionNo(request.versionNo().trim()); version.setStatus("DRAFT");
        version.setEffectiveDate(request.effectiveDate()); version.setChangeSummary(request.changeSummary()); version.setCurrentVersion(0); version.setSha256(file.getSha256()); version.setStoragePath(file.getStoragePath());
    ...
    private DocumentVersionDto publishVersionUnderLock(Long versionId) {
        ...
        if (!workflowProperties.isAllowSelfPublish() && version.getUploadedBy() != null && version.getUploadedBy().equals(TenantContext.userId())) {
            throw BusinessException.ruleBlock("DOCUMENT_DUTY_SEPARATION", "文件上传人与发布审批人不能为同一人", String.valueOf(TenantContext.userId()), "独立发布审批人", "由另一名技术负责人发布或开启流程参数");
        }
        ...
        version.setCurrentVersion(1); version.setStatus("PUBLISHED"); version.setPublishedBy(TenantContext.userId()); version.setPublishBy(TenantContext.username());
    ...
    @Override
    public DocumentVersionDto getVersion(Long versionId) { return toDocumentDto(requireDocument(versionId)); }

    @Override
    public org.springframework.core.io.Resource downloadVersion(Long versionId) {
        DocumentVersion version = requireDocument(versionId);
    ...
    public BomDto createBom(BomRequest request) {
        ...
        requireProject(request.projectId()); dataScope.requireProjectRole(request.projectId(), "TECHNICAL"); requireDocumentIfPresent(request.boundDocVersionId(), request.projectId());
        Bom bom = new Bom(); bom.setTenantId(TenantContext.tenantId()); bom.setProjectId(request.projectId()); bom.setBomNo(request.bomNo()); bom.setVersionNo(request.versionNo());
        bom.setBoundDocVersionId(request.boundDocVersionId()); bom.setStatus("DRAFT"); bomMapper.insert(bom);
        if (request.items() != null) for (BomItemRequest item : request.items()) {
            ...
            BomItem row = new BomItem(); row.setTenantId(TenantContext.tenantId()); row.setBomId(bom.getId()); row.setMaterialCode(item.materialCode()); row.setMaterialName(item.materialName());
            row.setSpecification(item.specification()); row.setQuantity(BigDecimal.valueOf(item.quantity())); row.setUnit(item.unit()); row.setSourceType(item.sourceType() == null ? "PURCHASE" : item.sourceType()); row.setSubstituteCode(item.substituteCode()); bomItemMapper.insert(row);
        }
        return toBomDto(bom);
    }
    ...
    public PageResult<ProcessRouteDto> processRoutes(Long projectId) {
        ...
        List<ProcessRouteDto> rows = routeMapper.selectList(Wrappers.<ProcessRoute>lambdaQuery().eq(projectId != null, ProcessRoute::getProjectId, projectId)
                ...
                .stream().map(this::toRouteDto).toList(); return new PageResult<>(rows, rows.size());
    }
    ...
    public ProcessRouteDto createProcessRoute(ProcessRouteRequest request) {
        ...
        requireProject(request.projectId()); dataScope.requireProjectRole(request.projectId(), "PROCESS"); requireDocumentIfPresent(request.boundDocVersionId(), request.projectId());
        ProcessRoute route = new ProcessRoute(); route.setTenantId(TenantContext.tenantId()); route.setProjectId(request.projectId()); route.setRouteNo(request.routeNo()); route.setVersionNo(request.versionNo()); route.setBoundDocVersionId(request.boundDocVersionId()); route.setStatus("DRAFT"); routeMapper.insert(route);
        if (request.steps() != null) for (ProcessStepRequest item : request.steps()) {
            ...
            ProcessStep row = new ProcessStep(); row.setTenantId(TenantContext.tenantId()); row.setRouteId(route.getId()); row.setStepNo(item.stepNo()); row.setStepName(item.stepName()); row.setWorkInstruction(item.workInstruction()); row.setEquipmentName(item.equipmentName()); row.setStandardHours(item.standardHours() == null ? null : BigDecimal.valueOf(item.standardHours())); row.setOutsourceFlag(Boolean.TRUE.equals(item.outsourceFlag()) ? 1 : 0); row.setInspectionPoint(Boolean.TRUE.equals(item.inspectionPoint()) ? 1 : 0); stepMapper.insert(row);
        }
    ...
    public PageResult<InspectionSpecDto> inspectionSpecs(Long projectId) {
        ...
        List<InspectionSpecDto> rows = inspectionSpecMapper.selectList(Wrappers.<InspectionSpec>lambdaQuery().eq(projectId != null, InspectionSpec::getProjectId, projectId)
                ...
                .stream().map(this::toInspectionDto).toList(); return new PageResult<>(rows, rows.size());
    }
    ...
    public InspectionSpecDto createInspectionSpec(InspectionSpecRequest request) {
        ...
        requireProject(request.projectId()); dataScope.requireProjectRole(request.projectId(), "PROCESS"); requireDocumentIfPresent(request.boundDocVersionId(), request.projectId());
        InspectionSpec spec = new InspectionSpec(); spec.setTenantId(TenantContext.tenantId()); spec.setProjectId(request.projectId()); spec.setSpecNo(request.specNo()); spec.setVersionNo(request.versionNo()); spec.setBoundDocVersionId(request.boundDocVersionId()); spec.setStatus("DRAFT"); inspectionSpecMapper.insert(spec);
        if (request.items() != null) for (InspectionItemRequest item : request.items()) {
            ...
            InspectionItem row = new InspectionItem(); row.setTenantId(TenantContext.tenantId()); row.setSpecId(spec.getId()); row.setItemName(item.itemName()); row.setStandardValue(item.standardValue()); row.setSamplingRule(item.samplingRule()); row.setAttachmentFileId(item.attachmentFileId()); inspectionItemMapper.insert(row);
        }
    ...
    private Map<String, Object> publishTechnicalPackageUnderLock(Long projectId) {
        ...
        bom.setStatus("PUBLISHED"); route.setStatus("PUBLISHED"); spec.setStatus("PUBLISHED"); bomMapper.updateById(bom); routeMapper.updateById(route); inspectionSpecMapper.updateById(spec);
        project.setCurrentDocVersionId(current.getId()); project.setStatus("TECH_PUBLISHED"); project.setStage("SAMPLE"); projectMapper.updateById(project);
        Map<String, Object> result = new LinkedHashMap<>(); result.put("projectId", projectId); result.put("documentVersionId", current.getId()); result.put("status", project.getStatus()); return result;
    }

    private Project requireProject(Long id) { Project project = projectMapper.selectById(id); if (project == null) throw new BusinessException("项目不存在"); dataScope.requireAccess(id); return project; }
    private DocumentVersion requireDocument(Long id) { DocumentVersion version = documentVersionMapper.selectById(id); if (version == null) throw new BusinessException("技术文件版本不存在"); dataScope.requireAccess(version.getProjectId()); return version; }
    private FileObject requireProjectFile(Long projectId, Long fileObjectId) { if (fileObjectId == null) throw new BusinessException("技术版本必须绑定已上传文件"); FileObject file = fileObjectMapper.selectById(fileObjectId); if (file == null || file.getTenantId() == null || file.getTenantId() != TenantContext.tenantId() || !projectId.equals(file.getProjectId())) throw BusinessException.accessDenied("FILE_PROJECT_SCOPE", "文件不属于当前项目", String.valueOf(fileObjectId), "当前项目文件", "重新上传项目文件"); return file; }
    private void requireDocumentIfPresent(Long id, Long projectId) { if (id != null && !projectId.equals(requireDocument(id).getProjectId())) throw new BusinessException("技术版本不属于当前项目"); }
    private boolean blank(String value) { return value == null || value.isBlank(); }
    private String projectNo(Long id) { return requireProject(id).getProjectNo(); }
    private DocumentVersionDto toDocumentDto(DocumentVersion x) { return new DocumentVersionDto(x.getId(), x.getProjectId(), x.getFileObjectId(), projectNo(x.getProjectId()), x.getFileName(), x.getFileType(), x.getVersionNo(), x.getStatus(), x.getEffectiveDate(), x.getChangeSummary(), x.getCurrentVersion() != null && x.getCurrentVersion() == 1, x.getSha256(), x.getFileObjectId() == null ? null : "/api/v1/admin/document-versions/" + x.getId() + "/download"); }
    private BomDto toBomDto(Bom x) { List<BomItemDto> items = bomItemMapper.selectList(Wrappers.<BomItem>lambdaQuery().eq(BomItem::getBomId, x.getId())).stream().map(item -> new BomItemDto(item.getId(), item.getMaterialCode(), item.getMaterialName(), item.getSpecification(), item.getQuantity().doubleValue(), item.getUnit(), item.getSourceType(), item.getSubstituteCode())).toList(); return new BomDto(x.getId(), x.getProjectId(), projectNo(x.getProjectId()), x.getBomNo(), x.getVersionNo(), x.getBoundDocVersionId(), x.getStatus(), items); }
    private ProcessRouteDto toRouteDto(ProcessRoute x) { List<ProcessStepDto> steps = stepMapper.selectList(Wrappers.<ProcessStep>lambdaQuery().eq(ProcessStep::getRouteId, x.getId()).orderByAsc(ProcessStep::getStepNo)).stream().map(item -> new ProcessStepDto(item.getId(), item.getStepNo(), item.getStepName(), item.getWorkInstruction(), item.getEquipmentName(), item.getStandardHours() == null ? null : item.getStandardHours().doubleValue(), item.getOutsourceFlag() != null && item.getOutsourceFlag() == 1, item.getInspectionPoint() != null && item.getInspectionPoint() == 1)).toList(); return new ProcessRouteDto(x.getId(), x.getProjectId(), projectNo(x.getProjectId()), x.getRouteNo(), x.getVersionNo(), x.getBoundDocVersionId(), x.getStatus(), steps); }
    private InspectionSpecDto toInspectionDto(InspectionSpec x) { List<InspectionItemDto> items = inspectionItemMapper.selectList(Wrappers.<InspectionItem>lambdaQuery().eq(InspectionItem::getSpecId, x.getId())).stream().map(item -> new InspectionItemDto(item.getId(), item.getItemName(), item.getStandardValue(), item.getSamplingRule(), item.getAttachmentFileId())).toList(); return new InspectionSpecDto(x.getId(), x.getProjectId(), projectNo(x.getProjectId()), x.getSpecNo(), x.getVersionNo(), x.getBoundDocVersionId(), x.getStatus(), items); }
}
