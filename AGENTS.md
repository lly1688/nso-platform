...
- nso-shared：通用模块，放置注解、枚举、异常、基础类、工具类
- nso-system：系统与业务模块，放置用户/角色/菜单/部门 Domain/Mapper/Service，以及 business/ 下九个核心业务域
- nso-framework：框架模块，放置 Security、JWT 认证、AOP、全局异常、Redis、存储、BaseController
- nso-scheduler：定时任务模块，放置风险扫描、临期提醒、超时升级、消息重试、报表汇总
- nso-scaffold：代码生成模块，放置表结构读取、模板引擎、代码预览与下载
- nso-admin：Spring Boot 启动与接口模块，集中放置 auth、system、business、publicapi Controller
- nso-web：Vue 3 + TypeScript + Element Plus PC 管理端
...
- `nso-shared` 不得依赖其他内部模块，保持轻量（不含 Spring Security、Redis、MinIO 等）。
- `nso-system` 只能依赖 `nso-shared`；如需缓存/锁/存储，通过 Port 接口由 `nso-framework` 实现。
- `nso-framework` 可以依赖 `nso-shared` 和 `nso-system`。
- `nso-admin` 作为启动和接口聚合层，依赖 `nso-framework`、`nso-system`、`nso-scheduler`、`nso-scaffold`。
...
- 接口前缀：`/api/v1/admin/**`（PC）、`/api/v1/public/**`（客户确认）。
- 所有业务异常使用统一错误码。
